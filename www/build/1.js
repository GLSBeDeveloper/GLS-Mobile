...
var ResetPasswordPage = /** @class */ (function (_super) {
    ...
    function ResetPasswordPage(auth, formBuilder, loading, navParams, notify, platform, viewCtrl) {
        ...
        _this.resetPwForm = _this.formBuilder.group((_a = {},
            _a[_this.fields.oldPassword.name] = new __WEBPACK_IMPORTED_MODULE_2__angular_forms__["b" /* FormControl */]('', __WEBPACK_IMPORTED_MODULE_2__angular_forms__["h" /* Validators */].required),
            _a[_this.fields.matchPassword.name] = new __WEBPACK_IMPORTED_MODULE_2__angular_forms__["c" /* FormGroup */]((_b = {},
                _b[_this.fields.newPassword.name] = new __WEBPACK_IMPORTED_MODULE_2__angular_forms__["b" /* FormControl */]('', __WEBPACK_IMPORTED_MODULE_2__angular_forms__["h" /* Validators */].required),
                _b[_this.fields.confirmPassword.name] = new __WEBPACK_IMPORTED_MODULE_2__angular_forms__["b" /* FormControl */]('', __WEBPACK_IMPORTED_MODULE_2__angular_forms__["h" /* Validators */].required),
                _b), function (formGroup) {
                return __WEBPACK_IMPORTED_MODULE_7__reset_password_validators_confirm_password_validator__["a" /* PasswordValidator */].areEqual(formGroup);
            }),
            _a));
        return _this;
        var _a, _b;
    }
    ResetPasswordPage.prototype.ionViewCanEnter = function () {
        if (__WEBPACK_IMPORTED_MODULE_3__globals__["a" /* Globals */].features.showChangePassword === false) { // do not enter page if feature is not enabled
            return false;
        }
    ...
    ResetPasswordPage.prototype.btnConfirm = function () {
        var _this = this;
        if (this.resetPwForm.invalid === true) {
            this.notify.error('Please resolve all validation errors.');
            return;
        }
        var oldPw = this.resetPwForm.get(this.fields.oldPassword.name).value;
        var newPw = this.resetPwForm.get(this.fields.matchPassword.name).get(this.fields.newPassword.name).value;
        var loader = this.loading.show();
        this.auth.changePassword(oldPw, newPw)
            .then(function () {
            loader.dismiss();
            _this.notify.success('Password reset successfully!');
            _this.auth.forceChangePassword = false; // stop change password dialog from opening again
            if (_this.isModal === true) {
                _this.viewCtrl.dismiss(); // close dialog if was opened as dialog
            }
            _this.resetPwForm.reset(); // clear the form
        })
            .catch(function (err) {
            loader.dismiss();
