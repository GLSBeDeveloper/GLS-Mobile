...
import { WebsocketEvent, WebsocketProvider } from '../../providers/websocket/websocket';
...
export class AuthProvider {
    ...
    login(username: string, password: string): Promise<void> {
        return this.authApi.login({ username: username, password: password })
            .then((userDetails: AuthResponse) => this.initUser(userDetails))
            .catch((err) => {
    ...
    isLoggedIn(): Promise<void> {
        return this.authApi.isLoggedIn()
            .then((userDetails) => this.initUser(userDetails))
            .catch((err) => {
                this.resetUser();
                throw err;
            });
