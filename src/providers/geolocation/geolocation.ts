...
export class GeolocationProvider {
    ...
    private readonly geolocationOptions: any = {
        ...
        pausesLocationUpdatesAutomatically: false, // <- in order to prevent the plugin automatically turning off location services when device is assumed to be stopped
    ...
    getCurrentLocation(): Promise<ILocation> {
        if (this.bgGeo !== undefined) { // for web usage
            ...
            return this.bgGeo.getCurrentPosition({ samples: 1 }).then((response) => ({
                coords: {
